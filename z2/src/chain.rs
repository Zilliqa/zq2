    ...
    pub fn get_forks(&self) -> Option<Vec<Value>> {
        match self {
            Chain::Zq2ProtoTestnet => Some(vec![
                ...
                json!({ "at_height": 11152000, "scilla_contract_creation_increments_account_balance": true, "serde_json_preserve_ordering": true }),
            ]),
            Chain::Zq2ProtoMainnet => Some(vec![
                ...
                // estimated: 2025-02-12T13:25:00Z
                json!({ "at_height": 7966800, "scilla_messages_can_call_evm_contracts": true, "scilla_contract_creation_increments_account_balance": true, "serde_json_preserve_ordering": true }),
            ]),
            _ => None,
        }
    }
