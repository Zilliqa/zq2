...
use zilliqa::{
    cfg::{
        ...
        total_native_token_supply_default, Amount, ConsensusConfig, NodeConfig,
    },
    crypto::SecretKey,
...
pub async fn get_implemented_jsonrpc_methods() -> Result<HashMap<ApiMethod, PageStatus>> {
    ...
    let my_node = Arc::new(Mutex::new(
        zilliqa::node::Node::new(config, secret_key, s1, s2, s3, s4, peers, None).await?,
    ));
