...
It will also have a noticeable impact on the node's performance while pruning is in progress i.e. it is possible that your node will fall in-and-out of sync from time to time.
...
1. At each epoch, the node increments its internal timestamp _ceiling_, which will result in any new state being tagged with a higher timestamp.
