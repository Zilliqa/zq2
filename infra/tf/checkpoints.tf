...
resource "google_compute_managed_ssl_certificate" "checkpoint" {
  ...
  managed {
    domains = var.alternative_subdomain == "" || var.alternative_subdomain == null ? [format("checkpoints.%s", var.subdomain)] : concat([format("checkpoints.%s", var.subdomain)], [format("checkpoints.%s", var.alternative_subdomain)])
  }
}
