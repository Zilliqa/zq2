  ...
  instances = flatten([
    for idx, node in var.config.nodes : [
      for n in range(node.count) : {
        ...
        snapshot        = lookup(try(var.config.disk_snapshot_override, {}), format("%s-%s-%s-%s", var.chain_name, lookup(local.labels, "private-api", var.role), var.region_mappings[(node.region != null ? node.region : ([for region in data.google_compute_zones.available : region if contains(region.names, node.zone)][0].region))], n), null)
