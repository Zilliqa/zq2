  ...
  instances = flatten([
    for idx, node in var.config.nodes : [
      for n in range(node.count) : {
        ...
        resource_name = format("%s-%s-%s-%s-%s", var.chain_name, var.role, (node.region != null ? node.region : ([for region in data.google_compute_zones.available : region if contains(region.names, node.zone)][0].region)), n, random_id.name_suffix.hex)
      }
    ]
  ])
