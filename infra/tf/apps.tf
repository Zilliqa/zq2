...
resource "google_compute_managed_ssl_certificate" "apps" {
  ...
  managed {
    domains = var.alternative_subdomain == "" || var.alternative_subdomain == null ? ["explorer.${var.subdomain}", "faucet.${var.subdomain}"] : concat(["explorer.${var.subdomain}", "faucet.${var.subdomain}"], ["explorer.${var.alternative_subdomain}", "faucet.${var.alternative_subdomain}"])
  }
}
