...
describe("ERC20 Is ZRC2", function () {
  ...
  it("Should be able to transfer to evm contract", async function () {
    const balanceBefore = await erc20_contract.balanceOf(erc20_contract.address);
    await (await erc20_contract.connect(contractOwner).transfer(erc20_contract.address, 150)).wait();
    const balanceAfter = await erc20_contract.balanceOf(erc20_contract.address);
    expect(balanceAfter).to.be.eq(balanceBefore + 150);
  });
  ...
  it("Should be able to receive scilla event via subscriptions", async function () {
    ...
    // TODO: I expect the logs have arrived at the subscription before we have a chance to query meaning this times out???
  ...
  it("Should not be able to transfer to evm contract when scilla receiver handler is present", async function () {
    const balanceBefore = await erc20_contract.balanceOf(erc165_contract.address);
    ...
    const balanceAfter = await erc20_contract.balanceOf(erc165_contract.address);
    expect(balanceAfter).to.be.eq(balanceBefore);
  });
});
