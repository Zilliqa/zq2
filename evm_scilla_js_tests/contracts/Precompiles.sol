...
contract Precompiles {
  ...
  function testRecovery(bytes32 documentHash, uint8 v, bytes32 r, bytes32 s) public pure returns (address) {
  ...
  function testSHA256(string memory word) public pure returns (bytes32) {
    bytes32 hash = sha256(bytes(word));
    return hash;
  }

  function testRipemd160(string memory word) public pure returns (bytes20) {
