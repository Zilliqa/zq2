//! Interface to the Scilla intepreter

use std::{
    collections::HashMap,
    fs::File,
    io::Write,
    net::{IpAddr, Ipv4Addr, SocketAddr},
    process::{Command, Stdio},
    str,
    sync::{
        mpsc::{channel, Receiver, Sender},
        Arc, Mutex,
    },
    thread::{self, JoinHandle},
    time::Duration,
};

use anyhow::{anyhow, Result};
use base64::Engine;
use jsonrpsee::{
    core::{client::ClientT, params::ObjectParams, ClientError},
    http_client::HttpClientBuilder,
    server::ServerHandle,
    types::{error::CALL_EXECUTION_FAILED_CODE, ErrorObject},
    RpcModule,
};
use primitive_types::{H160, H256};
use prost::Message as _;
use rand::{distributions::Alphanumeric, rngs::SmallRng, Rng, SeedableRng};
use serde::{
    de::{self, Unexpected},
    Deserialize, Deserializer,
};
use serde_json::Value;
use tempfile::TempDir;
use tokio::runtime;
use tracing::trace;

use crate::{
    scilla_proto::{Map, ProtoScillaQuery, ProtoScillaVal, ValType},
    serde_util::{bool_as_str, num_as_str},
    state::{ScillaValue, State},
};

/// The interface to the Scilla interpreter.
#[derive(Debug)]
pub struct Scilla {
    request_tx: Sender<(&'static str, ObjectParams)>,
    response_rx: Mutex<Receiver<Result<Value, ClientError>>>,
    state_server: Arc<Mutex<StateServer>>,
    temp_dir: TempDir,
    /// A guard which holds the [JoinHandle] for the scilla-server thread. On drop, we join the handle to ensure the
    /// thread can finish cleaning up (by running `docker stop`). Note that it is important this field exists after
    /// the sender and receiver channels, since we rely on those being dropped to signal to the thread that it should
    /// clean up.
    _guard: Guard,
}

#[derive(Debug)]
struct Guard(Option<JoinHandle<()>>);

impl Drop for Guard {
    fn drop(&mut self) {
        let _ = self.0.take().unwrap().join();
    }
}

impl Default for Scilla {
    fn default() -> Self {
        Self::new()
    }
}

impl Scilla {
    /// Create a new Scilla interpreter. This involves spawning two threads:
    /// 1. The server thread, responsible for creating, communicating with and destroying the server.
    /// 2. The state IPC thread, responsible for serving state requests from the running Scilla server.
    ///
    /// # Server thread
    ///
    /// This runs the Scilla server in a Docker container. All interaction with docker is done via the command line.
    /// This means having Docker installed and available in the `$PATH` is required.
    ///
    /// Once the docker container has been started, the thread starts an event loop which waits for JSON-RPC requests,
    /// forwards them to the server and sends the response back. Communication with the main thread is performed via
    /// two MPSC channels (one for requests and one for responses).
    ///
    /// If the other half of either channel is dropped by the main thread, we take that as our signal to terminate, by
    /// stopping the docker container.
    ///
    /// Note that the main thread holds a `JoinHandle` to this thread, so that it can wait for us to finish running
    /// `docker stop` before terminating.
    ///
    /// # State IPC thread
    ///
    /// This runs a JSON-RPC server at a random port. The address of this server is communicated to the Scilla server
    /// in each request (in the `-ipcaddress` argument). The server is implemented by [StateServer].
    ///
    /// After creating the [StateServer], we wrap it in an `Arc<Mutex<T>>` and send a clone back to the main thread,
    /// to enable shared access to the server.
    pub fn new() -> Scilla {
        let (request_tx, request_rx) = channel();
        let (response_tx, response_rx) = channel();

        let temp_dir = TempDir::new().unwrap();
        let temp_dir_path = temp_dir.path().to_owned();

        let handle = thread::spawn(move || {
            // Generate a name with a random suffix for the container.
            let mut name = "scilla-server-".to_owned();
            let rng = SmallRng::from_entropy();
            name.extend(rng.sample_iter(&Alphanumeric).map(char::from).take(8));

            let mut child = Command::new("docker")
                .arg("run")
                .arg("--name")
                .arg(&name)
                .arg("--add-host")
                .arg("host.docker.internal:host-gateway")
                // Let Docker auto-assign a free port on the host. The scilla-server listens on port 3000.
                .arg("--publish")
                .arg("3000")
                .arg("--volume")
                .arg(format!("{}:/scilla-files", temp_dir_path.display()))
                .arg("--init")
                .arg("--rm")
                .arg("scilla:b64-http") // TODO(BLOCKING): Use a published version
                .arg("/scilla/0/bin/scilla-server-http")
                .stdout(Stdio::null())
                .stderr(Stdio::null())
                .spawn()
                .unwrap();

            // Wait for the container to be running.
            for i in 0.. {
                let status_output = Command::new("docker")
                    .arg("inspect")
                    .arg("-f")
                    .arg("{{.State.Status}}")
                    .arg(&name)
                    .output()
                    .unwrap();
                let status = String::from_utf8(status_output.stdout).unwrap();
                if status.trim() == "running" {
                    break;
                }
                if i >= 20 {
                    panic!("container is still not running");
                }
                thread::sleep(Duration::from_millis(100));
            }

            // Find the port that Docker selected on the host.
            let inspect = Command::new("docker")
                .arg("inspect")
                .arg("--format")
                .arg("{{json .NetworkSettings.Ports}}")
                .arg(&name)
                .output()
                .unwrap();
            #[derive(Deserialize, Copy, Clone)]
            struct Addr {
                #[serde(rename = "HostIp")]
                ip: IpAddr,
                #[serde(rename = "HostPort", with = "num_as_str")]
                port: u16,
            }
            let inspect: HashMap<String, Vec<Addr>> =
                serde_json::from_slice(&inspect.stdout).unwrap();
            let addrs: Vec<SocketAddr> = inspect["3000/tcp"]
                .iter()
                .copied()
                .map(|a| (a.ip, a.port).into())
                .collect();
            let addr = addrs.iter().find(|a| a.is_ipv4()).unwrap();

            let runtime = runtime::Builder::new_current_thread()
                .enable_all()
                .build()
                .unwrap();
            let client = HttpClientBuilder::default()
                .request_timeout(Duration::from_secs(5))
                .build(format!("http://{addr}/run"))
                .unwrap();

            loop {
                let Ok((method, params)) = request_rx.recv() else {
                    break;
                };
                let response = runtime.block_on(client.request(method, params));
                let Ok(()) = response_tx.send(response) else {
                    break;
                };
            }

            Command::new("docker")
                .arg("stop")
                .arg("--signal")
                .arg("SIGKILL")
                .arg(&name)
                .stdout(Stdio::null())
                .stderr(Stdio::null())
                .spawn()
                .unwrap();
            let _ = child.wait();
        });

        let (tx, rx) = channel();
        thread::spawn(move || {
            let runtime = runtime::Builder::new_current_thread()
                .enable_all()
                .build()
                .unwrap();

            runtime.block_on(async {
                let server = StateServer::new().await.unwrap();
                let handle = server.handle.clone();
                let server = Arc::new(Mutex::new(server));
                tx.send(Arc::clone(&server)).unwrap();
                handle.stopped().await
            });
        });
        let state_server = rx.recv().unwrap();

        Scilla {
            request_tx,
            response_rx: Mutex::new(response_rx),
            state_server,
            temp_dir,
            _guard: Guard(Some(handle)),
        }
    }

    fn create_common_inputs(&mut self, code: &str, init: &[Value]) -> Result<()> {
        let contract_path = self.temp_dir.path().join("input.scilla");
        let mut f = File::create(contract_path)?;
        f.write_all(code.as_bytes())?;

        let init_path = self.temp_dir.path().join("init.json");
        let mut f = File::create(init_path)?;
        f.write_all(&serde_json::to_vec(&init)?)?;

        Ok(())
    }

    fn state_server_addr_from_container(&self) -> String {
        let addr = self.state_server.lock().unwrap().addr;

        format!("host.docker.internal:{}", addr.port())
    }

    pub fn check_contract(
        &mut self,
        code: &str,
        gas_limit: u64,
        init: &[Value],
    ) -> Result<Result<CheckOutput, Vec<String>>> {
        self.create_common_inputs(code, init)?;

        let args = vec![
            "-init".to_owned(),
            "/scilla-files/init.json".to_owned(),
            "-libdir".to_owned(),
            "/scilla/0/_build/default/src/stdlib/".to_owned(),
            "/scilla-files/input.scilla".to_owned(),
            "-gaslimit".to_owned(),
            gas_limit.to_string(),
            "-contractinfo".to_owned(),
            "-jsonerrors".to_owned(),
        ];

        let mut params = ObjectParams::new();
        params.insert("argv", args)?;

        self.request_tx.send(("check", params))?;
        let response = self.response_rx.lock().unwrap().recv()?;

        trace!(?response, "check response");

        match response {
            Ok(r) => Ok(Ok(serde_json::from_str(
                r.as_str()
                    .ok_or_else(|| anyhow!("response was not a string"))?,
            )?)),
            Err(ClientError::Call(e)) => {
                let error: CheckError = serde_json::from_str(e.message())?;
                Ok(Err(error
                    .errors
                    .into_iter()
                    .map(|e| e.error_message.trim().to_owned())
                    .collect()))
            }
            Err(e) => Err(anyhow!("{e:?}")),
        }
    }

    pub fn create_contract(
        &mut self,
        sender: H160,
        state: State,
        code: &str,
        gas_limit: u64,
        value: u128,
        init: &[Value],
    ) -> Result<(CreateOutput, H256)> {
        self.create_common_inputs(code, init)?;

        let args = vec![
            "-i".to_owned(),
            "/scilla-files/input.scilla".to_owned(),
            "-init".to_owned(),
            "/scilla-files/init.json".to_owned(),
            "-ipcaddress".to_owned(),
            self.state_server_addr_from_container(),
            "-gaslimit".to_owned(),
            gas_limit.to_string(),
            "-balance".to_owned(),
            value.to_string(),
            "-libdir".to_owned(),
            "/scilla/0/_build/default/src/stdlib/".to_owned(),
            "-jsonerrors".to_owned(),
        ];

        let mut params = ObjectParams::new();
        params.insert("argv", args)?;

        let (response, mut state) =
            self.state_server
                .lock()
                .unwrap()
                .active_call(sender, state, || {
                    self.request_tx.send(("run", params))?;
                    Ok(self.response_rx.lock().unwrap().recv()?)
                })?;

        let response: Value = match response {
            Ok(r) => r,
            Err(ClientError::Call(e)) => serde_json::from_str(e.message())?,
            Err(e) => {
                return Err(anyhow!("{e:?}"));
            }
        };

        trace!("Create response: {response}");

        // Sometimes Scilla returns a JSON object within a JSON string. Sometimes it doesn't...
        let response = if let Some(response) = response.as_str() {
            serde_json::from_str(response)?
        } else {
            serde_json::from_value(response)?
        };

        let root_hash = H256(state.root_hash()?.0);

        Ok((response, root_hash))
    }

    #[allow(clippy::too_many_arguments)]
    pub fn invoke_contract(
        &mut self,
        contract: H160,
        state: State,
        code: &str,
        gas_limit: u64,
        value: u128,
        init: &[Value],
        msg: &Value,
    ) -> Result<(InvokeOutput, H256)> {
        self.create_common_inputs(code, init)?;

        let message_path = self.temp_dir.path().join("input_message.json");
        let mut f = File::create(message_path)?;
        f.write_all(&serde_json::to_vec(&msg)?)?;

        let args = vec![
            "-init".to_owned(),
            "/scilla-files/init.json".to_owned(),
            "-ipcaddress".to_owned(),
            self.state_server_addr_from_container(),
            "-imessage".to_owned(),
            "/scilla-files/input_message.json".to_owned(),
            "-i".to_owned(),
            "/scilla-files/input.scilla".to_owned(),
            "-gaslimit".to_owned(),
            gas_limit.to_string(),
            "-balance".to_owned(),
            value.to_string(), // TODO: Should this be the sender's balance instead?
            "-libdir".to_owned(),
            "/scilla/0/_build/default/src/stdlib/".to_owned(),
            "-jsonerrors".to_owned(),
            "-pplit".to_owned(),
            "true".to_owned(),
        ];

        let mut params = ObjectParams::new();
        params.insert("argv", args)?;

        let (response, mut state) =
            self.state_server
                .lock()
                .unwrap()
                .active_call(contract, state, || {
                    self.request_tx.send(("run", params))?;
                    Ok(self.response_rx.lock().unwrap().recv()?)
                })?;

        let response: Value = match response {
            Ok(r) => r,
            Err(ClientError::Call(e)) => serde_json::from_str(e.message())?,
            Err(e) => {
                return Err(anyhow!("{e:?}"));
            }
        };

        trace!("Invoke response: {response}");

        if response
            .as_object()
            .map(|obj| obj.contains_key("errors"))
            .unwrap_or(false)
        {
            return Err(anyhow!("call failed: {response}"));
        }

        // Sometimes Scilla returns a JSON object within a JSON string. Sometimes it doesn't...
        let response = if let Some(response) = response.as_str() {
            serde_json::from_str(response)?
        } else {
            serde_json::from_value(response)?
        };

        let root_hash = H256(state.root_hash()?.0);

        Ok((response, root_hash))
    }
}

#[derive(Debug, Deserialize)]
pub struct CheckOutput {
    pub contract_info: ContractInfo,
}

#[derive(Debug, Deserialize)]
struct CheckError {
    errors: Vec<Error>,
}

#[derive(Debug, Deserialize)]
struct Error {
    pub error_message: String,
}

#[derive(Debug, Deserialize)]
pub struct ContractInfo {
    pub scilla_major_version: String,
    pub fields: Vec<Param>,
}

#[derive(Debug, Deserialize)]
pub struct Param {
    #[serde(rename = "vname")]
    pub name: String,
    pub depth: u64,
    #[serde(rename = "type")]
    pub ty: String,
}

#[derive(Debug, Deserialize)]
pub struct CreateOutput {
    #[serde(with = "num_as_str")]
    pub gas_remaining: u64,
}

#[derive(Debug, Deserialize)]
pub struct InvokeOutput {
    #[serde(rename = "_accepted", with = "bool_as_str")]
    pub accepted: bool,
    #[serde(default)]
    pub messages: Vec<Message>,
    #[serde(default)]
    pub events: Vec<ScillaEvent>,
}

#[derive(Debug, Deserialize)]
pub struct ScillaEvent {
    #[serde(rename = "_eventname")]
    pub event_name: String,
    pub params: Vec<ParamValue>,
}

#[derive(Debug, Deserialize)]
pub struct ParamValue {
    #[serde(rename = "vname")]
    pub name: String,
    pub value: String,
    #[serde(rename = "type")]
    pub ty: String,
}

#[derive(Debug, Deserialize)]
pub struct Message {
    #[serde(rename = "_tag")]
    pub tag: String,
    #[serde(rename = "_amount")]
    pub amount: String,
    #[serde(rename = "_recipient")]
    pub recipient: H160,
    pub params: Value,
}

#[derive(Debug)]
struct StateServer {
    addr: SocketAddr,
    handle: ServerHandle,
    /// This should be `Some` when a call is being made to the Scilla server. It stores the current contract address
    /// and state.
    active_call: Arc<Mutex<Option<ActiveCall>>>,
}

impl StateServer {
    async fn new() -> Result<StateServer> {
        let server = jsonrpsee::server::Server::builder()
            .build((Ipv4Addr::UNSPECIFIED, 0))
            .await?;
        let addr = server.local_addr()?;

        let mut module = RpcModule::new(());

        fn de_b64<'de, D: Deserializer<'de>>(d: D) -> Result<Vec<u8>, D::Error> {
            let b64 = base64::engine::general_purpose::STANDARD;

            let s = String::deserialize(d)?;
            b64.decode(&s)
                .map_err(|_| de::Error::invalid_value(Unexpected::Str(&s), &"a base64 string"))
        }

        fn err(e: impl ToString) -> ErrorObject<'static> {
            ErrorObject::owned(CALL_EXECUTION_FAILED_CODE, e.to_string(), None::<()>)
        }

        let active_call: Arc<Mutex<Option<ActiveCall>>> = Arc::new(Mutex::new(None));

        module.register_method("fetchStateValueB64", {
            let active_call = Arc::clone(&active_call);
            let b64 = base64::engine::general_purpose::STANDARD;
            move |params, ()| {
                #[derive(Deserialize)]
                struct Params {
                    #[serde(deserialize_with = "de_b64")]
                    query: Vec<u8>,
                }

                let Params { query } = params.parse()?;
                let ProtoScillaQuery { name, indices, .. } =
                    ProtoScillaQuery::decode(query.as_slice()).map_err(err)?;

                let mut active_call = active_call.lock().unwrap();
                let Some(active_call) = active_call.as_mut() else {
                    return Err(err("no active call"));
                };

                let value = active_call
                    .fetch_state_value(name, indices)
                    .map_err(err)?;

                let result = match value {
                    Some(v) => vec![true.into(), b64.encode(v.encode_to_vec()).into()],
                    None => vec![false.into(), String::new().into()],
                };

                Ok(Value::Array(result))
            }
        })?;
        module.register_method("fetchExternalStateValueB64", {
            let active_call = Arc::clone(&active_call);
            let b64 = base64::engine::general_purpose::STANDARD;
            move |params, ()| {
                #[derive(Deserialize)]
                struct Params {
                    addr: H160,
                    #[serde(deserialize_with = "de_b64")]
                    query: Vec<u8>,
                }

                let Params { addr, query } = params.parse()?;
                let ProtoScillaQuery { name, indices, .. } =
                    ProtoScillaQuery::decode(query.as_slice()).map_err(err)?;

                let mut active_call = active_call.lock().unwrap();
                let Some(active_call) = active_call.as_mut() else {
                    return Err(err("no active call"));
                };

                let value = active_call
                    .fetch_external_state_value(addr, name, indices)
                    .map_err(err)?;

                let result = match value {
                    Some((v, ty)) => {
                        vec![true.into(), b64.encode(v.encode_to_vec()).into(), ty.into()]
                    }
                    None => vec![false.into(), String::new().into(), String::new().into()],
                };

                Ok(Value::Array(result))
            }
        })?;
        module.register_method("updateStateValueB64", {
            let active_call = Arc::clone(&active_call);
            move |params, ()| {
                #[derive(Deserialize)]
                struct Params {
                    #[serde(deserialize_with = "de_b64")]
                    query: Vec<u8>,
                    #[serde(deserialize_with = "de_b64")]
                    value: Vec<u8>,
                }

                let Params { query, value } = params.parse()?;
                let ProtoScillaQuery {
                    name,
                    mapdepth: _,
                    indices,
                    ignoreval,
                } = ProtoScillaQuery::decode(query.as_slice()).map_err(err)?;
                let ProtoScillaVal { val_type } =
                    ProtoScillaVal::decode(value.as_slice()).map_err(err)?;
                let val_type = val_type.ok_or_else(|| err("no val_type"))?;

                let mut active_call = active_call.lock().unwrap();
                let Some(active_call) = active_call.as_mut() else {
                    return Err(err("no active call"));
                };

                match active_call.update_state_value(name, indices, ignoreval, val_type) {
                    Ok(()) => Ok(()),
                    Err(e) => Err(err(e)),
                }
            }
        })?;
        module.register_method("fetchBlockchainInfo", {
            let active_call = Arc::clone(&active_call);
            move |params, ()| {
                #[derive(Deserialize)]
                struct Params {
                    query_name: String,
                    query_args: String,
                }

                let Params {
                    query_name,
                    query_args,
                } = params.parse()?;

                let mut active_call = active_call.lock().unwrap();
                let Some(active_call) = active_call.as_mut() else {
                    return Err(err("no active call"));
                };

                active_call.fetch_blockchain_info(query_name, query_args);

                Ok::<_, ErrorObject>(())
            }
        })?;

        let handle = server.start(module);

        Ok(StateServer {
            addr,
            handle,
            active_call,
        })
    }

    fn active_call<R>(
        &mut self,
        sender: H160, // TODO: rename
        state: State,
        f: impl FnOnce() -> Result<R>,
    ) -> Result<(R, State)> {
        {
            let mut active_call = self.active_call.lock().unwrap();
            *active_call = Some(ActiveCall { sender, state });
        }

        let response = f()?;

        let mut active_call = self.active_call.lock().unwrap();
        let ActiveCall { state, .. } = active_call.take().unwrap();
        Ok((response, state))
    }
}

fn convert_scilla_value(value: &ScillaValue) -> ProtoScillaVal {
    match value {
        ScillaValue::Bytes(bytes) => ProtoScillaVal {
            val_type: Some(ValType::Bval(bytes.clone())),
        },
        ScillaValue::Map(map) => {
            let m = map
                .iter()
                .map(|(k, v)| (k.clone(), convert_scilla_value(v)))
                .collect();
            ProtoScillaVal {
                val_type: Some(ValType::Mval(Map { m })),
            }
        }
    }
}

#[derive(Debug)]
struct ActiveCall {
    sender: H160,
    state: State,
}

impl ActiveCall {
    fn fetch_value_inner(
        &self,
        addr: H160,
        name: String,
        indices: Vec<Vec<u8>>,
    ) -> Result<Option<(ProtoScillaVal, String)>> {
        let account = self.state.get_account(addr)?;
        let storage = account
            .contract
            .scilla_storage()
            .ok_or_else(|| anyhow!("not a scilla contract"))?;

        let Some((value, ty)) = storage.get(&name) else {
            return Ok(None);
        };
        let mut value = value;

        for index in &indices {
            let index = str::from_utf8(index)?;
            let Some(map) = value.as_map() else {
                return Err(anyhow!("not a map"));
            };
            let Some(inner) = map.get(index) else {
                return Ok(None);
            };
            value = inner;
        }

        Ok(Some((convert_scilla_value(value), ty.clone())))
    }

    fn fetch_state_value(
        &self,
        name: String,
        indices: Vec<Vec<u8>>,
    ) -> Result<Option<ProtoScillaVal>> {
        Ok(self
            .fetch_value_inner(self.sender, name, indices)?
            .map(|(v, _)| v))
    }

    fn fetch_external_state_value(
        &self,
        addr: H160,
        name: String,
        indices: Vec<Vec<u8>>,
    ) -> Result<Option<(ProtoScillaVal, String)>> {
        fn scilla_val(b: Vec<u8>) -> ProtoScillaVal {
            ProtoScillaVal {
                val_type: Some(ValType::Bval(b)),
            }
        }

        let account = self.state.get_account(addr)?;
        match name.as_str() {
            "_balance" => {
                let val = scilla_val(format!("\"{}\"", account.balance).into_bytes());
                Ok(Some((val, "Uint128".to_owned())))
            }
            "_nonce" => {
                let val = scilla_val(format!("\"{}\"", account.nonce + 1).into_bytes());
                Ok(Some((val, "Uint64".to_owned())))
            }
            "_this_address" => {
                let val = scilla_val(format!("\"0x{:#x}\"", addr).into_bytes());
                Ok(Some((val, "ByStr20".to_owned())))
            }
            "_codehash" => {
                todo!()
            }
            _ => self.fetch_value_inner(addr, name, indices),
        }
    }

    fn update_state_value(
        &mut self,
        name: String,
        indices: Vec<Vec<u8>>,
        ignore_value: bool,
        val_type: ValType,
    ) -> Result<()> {
        let mut account = self.state.get_account(self.sender)?;
        let storage = account
            .contract
            .scilla_storage_mut()
            .ok_or_else(|| anyhow!("not a scilla contract"))?;
        let Some((value, _)) = storage.get_mut(&name) else {
            return Ok(());
        };

        if ignore_value {
            if indices.is_empty() {
                return Err(anyhow!("indices cannot be empty"));
            }

            let Some(mut map) = value.as_map_mut() else {
                return Err(anyhow!("not a map"));
            };

            for index in &indices[..(indices.len() - 1)] {
                let index = str::from_utf8(index)?;
                let Some(inner) = map.get_mut(index) else {
                    return Ok(());
                };
                let Some(inner) = inner.as_map_mut() else {
                    return Err(anyhow!("not a map"));
                };
                map = inner;
            }

            let last = &indices[indices.len() - 1];
            let last = str::from_utf8(last)?;
            map.remove(last);
        } else {
            let mut value = value;
            for index in &indices {
                let index = str::from_utf8(index)?;
                let Some(map) = value.as_map_mut() else {
                    return Err(anyhow!("not a map"));
                };
                let inner = map.entry(index.to_owned()).or_insert_with(ScillaValue::map);
                value = inner;
            }
            let val_type = match val_type {
                ValType::Bval(bytes) => ScillaValue::Bytes(bytes),
                ValType::Mval(_) => todo!(),
            };
            *value = val_type;
        }

        self.state.save_account(self.sender, account)?;

        Ok(())
    }

    fn fetch_blockchain_info(&self, name: String, args: String) {
        eprintln!("fetch_blockchain_info - {name}, {args}");
    }
}
