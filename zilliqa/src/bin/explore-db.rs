...
fn print_db_stats(stats: DatabaseStats) {
    ...
    eprintln!(
        "stored_bytes: {}",
        ByteSize::b(stats.stored_bytes()).display()
    );
    eprintln!(
        "metadata_bytes: {}",
        ByteSize::b(stats.metadata_bytes()).display()
    );
    eprintln!(
        "fragmented_bytes: {}",
        ByteSize::b(stats.fragmented_bytes()).display()
    );
    eprintln!(
        "page_size: {}",
        ByteSize::b(stats.page_size() as u64).display()
    );
    eprintln!();
}
