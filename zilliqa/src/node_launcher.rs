                            ...
                            request_response::Event::Message{peer, message} => {
                                match message {
                                    request_response::Message::Request {request_id, request, channel} => {
                                        ...
                                        let fileTmp = vec![0,1,2];
                                        //swarm.behaviour_mut().send_response(channel, FileResponse(fileTmp));
                                        swarm.behaviour_mut().request_response.send_response(channel, FileResponse(fileTmp));
                                    }
...
use libp2p::kad::{GetProvidersOk, /*Kademlia, KademliaEvent,*/ QueryId};
//use libp2p::multiaddr::Protocol;
