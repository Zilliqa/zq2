...
use redb::{
    ...
    ReadTransaction, ReadableMultimapTable, ReadableTable, ReadableTableMetadata, Table,
...
// a tuple of the opened `redb` tables.
    ...
    T4: ReadableTable<u64, bool>,
{
    pub fn by_view(&self, view: u64) -> Result<Option<Block>> {
        let Some(block) = self.0 .0.get(view)? else {
            return Ok(None);
        };
    ...
    pub fn min_by_view(&self) -> Result<Option<Block>> {
        let Some((_, block)) = self.0 .0.first()? else {
            return Ok(None);
        };
        Ok(Some(bincode::deserialize(&block.value())?))
    }
    ...
    pub fn by_hash(&self, hash: Hash) -> Result<Option<Block>> {
        let Some(view) = self.0 .1.get(&hash.0)? else {
            return Ok(None);
        };
    ...
    pub fn canonical_by_height(&self, height: u64) -> Result<Option<Block>> {
        for view in self.0 .2.get(height)? {
    ...
    )>
{
    pub fn insert(&mut self, block: &Block) -> Result<()> {
        self.0 .0.insert(block.view(), bincode::serialize(block)?)?;
        self.0 .1.insert(&block.hash().0, block.view())?;
        self.0 .2.insert(block.number(), block.view())?;
        self.0 .3.insert(block.view(), true)?;
        Ok(())
    }

    pub fn delete(&mut self, view: u64) -> Result<()> {
        let Some(block) = self.0 .0.remove(view)? else {
            return Ok(());
        };
        let block: Block = bincode::deserialize(&block.value())?;
        self.0 .1.remove(&block.hash().0)?;
        self.0 .2.remove(block.number(), block.view())?;
        self.0 .3.remove(block.view())?;
        Ok(())
    }

    pub fn set_canonical(&mut self, view: u64) -> Result<()> {
        self.0 .3.insert(view, true)?;
        Ok(())
    }

    pub fn set_non_canonical(&mut self, view: u64) -> Result<()> {
        self.0 .3.insert(view, false)?;
        Ok(())
    }
}
...
pub struct TouchedAddressIndex<T>(T);
    ...
    T2: ReadableTable<&'static [u8; 32], (u64, &'static [u8; 20])>,
{
    pub fn get(&self, address: Address) -> Result<Vec<Hash>> {
        let hashes = self.0 .0.get(&<[u8; 20]>::from(address))?;
        hashes
    ...
    )>
{
    pub fn insert(&mut self, address: Address, txn_hash: Hash) -> Result<()> {
        ...
        self.0 .0.insert(key, (next_index, &txn_hash.0))?;
        self.0 .1.insert(&txn_hash.0, (next_index, key))?;
        Ok(())
    }

    pub fn delete_by_txn_hash(&mut self, txn_hash: Hash) -> Result<()> {
        let Some(value) = self.0 .1.remove(&txn_hash.0)? else {
            return Ok(());
        };
        let (index, address) = value.value();
        self.0 .0.remove(address, (index, &txn_hash.0))?;
        Ok(())
    }
}
...
const VIEW: TableDefinition<(), u64> = TableDefinition::new("view");

pub struct ViewTable<T>(T);

impl<T: ReadableTable<(), u64>> ViewTable<T> {
    pub fn get(&self) -> Result<Option<u64>> {
        Ok(self.0.get(())?.map(|v| v.value()))
    }
}

impl ViewTable<Table<'_, (), u64>> {
    /// Sets the provided view if it is greater than the existing view. Returns true if the value was updated.
    pub fn set(&mut self, view: u64) -> Result<bool> {
...
impl Db {
    ...
    pub fn write(&self) -> Result<TxWrite> {
        Ok(TxWrite(self.db.begin_write()?))
    }

    /// Like [Db::write] but writes will only be eventually durable.
    pub fn write_non_durable(&self) -> Result<TxWrite> {
...
impl TxRead {
    ...
    > {
        let blocks = self.0.open_table(BLOCKS)?;
        let blocks_hash_index = self.0.open_table(BLOCKS_HASH_INDEX)?;
        let blocks_height_index = self.0.open_multimap_table(BLOCKS_HEIGHT_INDEX)?;
        let block_is_canonical = self.0.open_table(BLOCK_IS_CANONICAL)?;
        Ok(BlocksTable((
            blocks,
            blocks_hash_index,
            blocks_height_index,
    ...
    > {
        let index = self.0.open_multimap_table(TOUCHED_ADDRESS_INDEX)?;
        let reverse_index = self.0.open_table(TOUCHED_ADDRESS_REVERSE_INDEX)?;
        Ok(TouchedAddressIndex((index, reverse_index)))
    }
    ...
    pub fn view(&self) -> Result<ViewTable<ReadOnlyTable<(), u64>>> {
...
impl TxWrite {
    ...
    pub fn view(&self) -> Result<ViewTable<Table<(), u64>>> {
        Ok(ViewTable(self.0.open_table(VIEW)?))
    }
