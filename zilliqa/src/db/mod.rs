...
impl Db {
        ...
        P: AsRef<Path>,
    {
        ...
        // Start a new thread to periodically flush database changes to disk. All our normal DB writes use
        // `Durability::None`, meaning changes are not persisted to disk. By committing an empty write transaction, we
        // flush any pending changes to disk.
        let inner_db = db.db.clone();
        std::thread::spawn(move || {
            loop {
                std::thread::sleep(Duration::from_secs(1));
                if let Ok(write) = inner_db.begin_write() {
                    if write.commit().is_err() {
                        error!("failed to commit")
                    }
                } else {
                    error!("failed to open write transaction");
                }
            }
        });
