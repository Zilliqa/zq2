...
impl eth_trie::DB for TrieStorage {
    ...
    fn get(&self, key: &[u8]) -> Result<Option<Vec<u8>>> {
        ...
        let value = self.db.read()?.state_trie()?.get(key)?;

        let mut cache = self.db.state_cache.lock().unwrap();
    ...
    fn insert_batch(&self, keys: Vec<Vec<u8>>, values: Vec<Vec<u8>>) -> Result<(), Self::Error> {
        ...
        for (key, value) in keys.into_iter().zip(values) {
            write.state_trie()?.insert(&key, &value)?;
            let _ = self.db.state_cache.lock().unwrap().insert(key, value);
        }
        write.commit()?;
