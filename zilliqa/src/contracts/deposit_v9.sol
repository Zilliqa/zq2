...
contract Deposit is Initializable, UUPSUpgradeable {
    ...
    function leaderAtView(
        ...
        // as long as their number does not exceed 256 (bit indices 0..255 of a uint256).
        // Must stay consistent with `maximumStakers` (the deposit cap), otherwise a full
        // committee would make leader selection revert on every view and halt the chain.
    ...
    function leaderAtViewWithRandao(
        ...
        uint256 randomness = uint256(
            keccak256(
                bytes.concat(bytes32(block.prevrandao), bytes32(viewNumber))
            )
        );
        uint256 bitmap;
        uint256 number = committee().stakerKeys.length;
    ...
    function deposit(
        ...
        // A staker's `_stakersMap` entry (including its pending withdrawal queue) persists
        // forever, even after a full unstake removes it from the committee. Re-using the same
        // `blsPubKey` via deposit() must not let a different caller silently overwrite the
        // control address and take over that lingering entry (and drain its withdrawals).
        // Only the existing control address may re-deposit under a known key; a brand new key
        // (controlAddress == 0) is unrestricted.
        if (
