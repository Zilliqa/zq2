...
impl Consensus {
    ...
    fn apply_rewards(
        ...
        Self::apply_rewards_at(state, config, committee, proposer, view, cosigned)
    }
    ...
    fn apply_rewards_at(
        ...
        if let Some(proposer_address) = at_state.get_reward_address(proposer)? {
        ...
        let cosigner_stake: Vec<_> = committee
            ...
            .map(|(_, &pub_key)| {
                let reward_address = at_state.get_reward_address(pub_key).unwrap();
                let stake = at_state.get_stake(pub_key).unwrap().unwrap().get();
