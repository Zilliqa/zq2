...
impl Consensus {
    ...
    // Optionally returns a network message that should be sent as the result of this newly received block. This occurs
    // when the node has buffered votes for a block it doesn't know about and later receives that block, resulting in a
    // new block proposal.
    pub fn receive_block(&mut self, proposal: Proposal) -> Result<(bool, Option<NetworkMessage>)> {
        ...
        match self.check_block(&block) {
            Ok(()) => {
                ...
                Ok((was_new, response.filter(|_| was_new)))
            }
            Err(e) => {
