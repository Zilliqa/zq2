...
use tokio::sync::mpsc;
use zilliqa::{
    cfg::{Amount, GenesisDeposit, NodeConfig}, consensus::Consensus, crypto::{Hash, SecretKey}, db::Db, exec::zil_contract_address, message::{Block, ExternalMessage, Proposal, QuorumCertificate, Vote, MAX_COMMITTEE_SIZE}, node::{MessageSender, RequestId}, schnorr, sync::SyncPeers, test_util::compile_contract, time::{self, SystemTime}, transaction::{
...
fn scilla_contract_benchmark(c: &mut Criterion, name: &str, code: String, call: String, call_gas_limit: u64, txns_per_block: usize) {
    let signer = schnorr::SecretKey::random(&mut rand::thread_rng());
    let key = signer.public_key();
    let hashed = Sha256::digest(key.to_encoded_point(true).as_bytes());
    let address = Address::from_slice(&hashed[12..]);
