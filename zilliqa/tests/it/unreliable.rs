use zilliqa::constants::{LAG_BEHIND_CURRENT_VIEW, MISSED_VIEW_THRESHOLD};
...
async fn block_production_even_when_lossy_network(mut network: Network) {
    ...
    let until_at = LAG_BEHIND_CURRENT_VIEW + MISSED_VIEW_THRESHOLD as u64;

    // wait until at least 5 blocks have been produced
    network
        .run_until_block_finalized(until_at, 400)
        .await
        .unwrap();
...
async fn blocks_are_produced_while_a_node_restarts(mut network: Network) {
    ...
    // Produce more blocks.
    network
        .run_until_block(&other_wallet, until_at + 5, 400)
        .await;
    ...
    network.run_until_block(&wallet, until_at + 10, 400).await;
}
